...
from policy_corpus_builder.adapters.eurlex_nim import (
...
class EurlexNIMAdapter:
    ...
    def validate_source_config(self, source: SourceConfig, *, base_path: Path) -> None:
        ...
        _resolve_optional_positive_int(settings, "nim_max_pages")
        ...
        _resolve_timeout_tuple(settings)
    ...
    *,
    ...
    if _require_bool(settings, "fetch_full_text", default=True):
        fulltext_df = batch_fetch_nim_fulltext(
            ...
            timeout=_resolve_timeout_tuple(settings),
...
def _retrieve_nim_rows(acts_df: pd.DataFrame, settings: dict[str, Any]) -> pd.DataFrame:
    ...
    for row in acts_df.to_dict(orient="records"):
        ...
        page_df = get_national_transpositions_by_celex_ws(
            ...
            max_pages=_resolve_optional_positive_int(settings, "nim_max_pages"),
...
def _resolve_optional_positive_int(settings: dict[str, Any], key: str) -> int | None:
...
def _resolve_timeout_tuple(settings: dict[str, Any]) -> tuple[int, int]:
