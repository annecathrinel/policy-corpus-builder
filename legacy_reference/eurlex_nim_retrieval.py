...
def build_eligible_act_table(doc_scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for rec in doc_scores.itertuples(index=False):
        ...
        rows.append(
            {
                "celex": celex,
                "eu_act_title": getattr(rec, "title", ""),
                "eu_act_type": getattr(rec, "celex_doc_type_label", info.descriptor_label or ""),
                "descriptor": info.descriptor,
                "year": info.year,
                "ecological_ambition_score": getattr(rec, "ecological_ambition_score", None),
                "design_specificity_score": getattr(rec, "design_specificity_score", None),
                "governance_gap_score": getattr(rec, "governance_gap_score", None),
    ...
    return (
        ...
        .sort_values(["year", "celex"], ascending=[False, True])
